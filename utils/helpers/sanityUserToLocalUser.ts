import { LocalUser, User } from "@/typess/user";
  ...
  if (sanityUser) {
    return {
      ...
      avatar: {
        asset: {
          url:
            (sanityUser.avatar && sanityUser.avatar.asset.url) || "/person.jpg",
        },
      },
      ...
      recentVisited: sanityUser.recentVisited
        ...
        : [],
      cart: sanityUser.cart ? sanityUser.cart.map((item) => item.item._id) : [],
